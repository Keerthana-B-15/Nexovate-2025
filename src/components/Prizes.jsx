...
const PrizeCard = ({ prize, index, isCenter }) => (
  <div className={`group relative ${isCenter ? 'md:-mt-4 md:scale-110' : ''} transition-all duration-500 hover:scale-105`}>
    ...
    {/* Main Card */}
    <div className={`relative bg-accent rounded-xl p-6 border border-gray-700/50 group-hover:border-primary/40 transition-all duration-300 shadow-2xl ${prize.borderGlow} group-hover:shadow-2xl backdrop-blur-sm overflow-visible min-h-[320px]`}>
      ...
      <div className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        <div className={`w-16 h-16 rounded-full bg-gradient-to-br ${prize.gradient} flex items-center justify-center shadow-lg border-4 border-accent`}>
      ...
      <div className="relative z-10 text-center pt-6">
        ...
        <div className="mb-4 flex justify-center">
          <Trophy className={`w-10 h-10 text-primary group-hover:text-[#dddf23] transition-colors duration-300 group-hover:scale-110 group-hover:rotate-12`} />
        </div>
        ...
        <div className="mb-3">
          ...
          <div className="w-12 h-1 bg-gradient-to-r from-primary to-[#dddf23] rounded-full mx-auto"></div>
        </div>
    ...
    <div className="grid md:grid-cols-3 gap-8 items-stretch">
      {prizes.map((prize, index) => (
        <PrizeCard 
          ...
          prize={prize} 
          index={index}
          isCenter={index === 0} // Make 1st prize the center highlight
        />
      ))}
    </div>
